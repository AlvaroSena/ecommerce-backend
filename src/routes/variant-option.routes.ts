...
const variantOptionService = new VariantOptionService(variantOptionRepository, productVariantRepository);
const variantOptionController = new VariantOptionController(variantOptionService);

variantOptionRoutes.post("/", restVerifyToken, (request: Request, response: Response, next: NextFunction) =>
  variantOptionController.postVariantOption(request, response, next),
);

variantOptionRoutes.get("/", (request: Request, response: Response, next: NextFunction) =>
  variantOptionController.getVariantOptions(request, response, next),
);

variantOptionRoutes.get("/:id", (request: Request, response: Response, next: NextFunction) =>
  variantOptionController.getVariantOption(request, response, next),
);

variantOptionRoutes.put("/update/:id", restVerifyToken, (request: Request, response: Response, next: NextFunction) =>
