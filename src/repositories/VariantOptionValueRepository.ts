...
export class VariantOptionValueRepository
  implements IVariantOptionValueRepository
{
  ...
  async create(
    ...
    const result = await db
      .insert(variantOptionsValues)
      .values({
        value: variantOptionValue.getValue(),
        isSoldOut: variantOptionValue.getIsSoldOut(),
        variantOptionId: variantOptionValue.getVariantOptionId(),
      })
      .returning();
  ...
  async update(
    ...
    const result = await db
      ...
      .set({
        ...
        isSoldOut: productVariant.getIsSoldOut(),
      })
      .where(eq(variantOptionsValues.id, id))
      .returning();
